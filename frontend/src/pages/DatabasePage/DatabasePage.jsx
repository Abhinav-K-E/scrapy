...
const DatabasePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchAllData("scraps_db");
      setProducts(data);
      console.log(data);
    };
    fetchData();
  ...
  const handleKeyDown = async (event) => {
    if (event.keyCode === 13) {
      ...
      if (search.length > 0) {
        const res = await fetchAxios.get(`/scraps/${search}`);
        setProducts(res.data);
      } else {
        const res = await fetchAxios.get(`/scraps/.*`);
        setProducts(res.data);
      }
      setLoader(false);
    }
        ...
        <div className="database-cards">
          {products?.map((item) => (
            <div className="database-card">
              <img src={`${item?.imageUrl}`} alt="img" className="card-left" />
